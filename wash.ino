...
const int S-auto = 10;	// Switch auto/manual????????????? ????-?????? ??????????
const int L-cold = 9;		// Led cold????????? ?????????? ???????? ????
const int L-hot = 8;		// Led hot????????? ?????????? ??????? ????
const int L-boil = 7;		// Led boiler????????? ????????? ???????
const int L-auto = 6;		// Led auto????????? ????
const int RelCold = 5;		// Relay cold???? 1 - ???????? ????
const int RelHot = 4;		// Relay hot???? 2 - ??????? ????
const int RelBoil = 3;		// Relay boiler???? 3 - ???????????? ?????? ???? ? ????????? ???????
const int RelOn = 2;		// Relay power on???? 4 - ??????? ?? ?????????? ??? ????
const int PressMin = 100;	// Pressure min??????????? ????????
unsigned long DelaySleep = 600000; // 10 min test cicle
int PressCold;				// Pressure cold???????? ???????? ????
int pressHot;					// Pressure hot???????? ??????? ????
int SenCold = 0;			//  Sensor cold A0 - ?????? ???????? ???????? ????
int SenHot = 1;				// Sensor hot A1 - ?????? ???????? ??????? ????
int DelaySwitch = 10000;	//  Delay time for switch???????? ????? ????????? ????
